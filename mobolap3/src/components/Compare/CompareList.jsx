...
const CompareList = ({ products: data }) => {
  ...
  const RemoveFromCompare = (id, product) => {
    if (!product) return;
    console.log("ID: ", id, " Prod: ", product);
  ...
  const HandleSelectedProduct = (id, product) => {
    if (products.length === MAX_PRODUCTS) {
      toast.error(notify("failedToAddToCompare"));
      setOpen(false);
      return;
    }
    const isFound = products.find((prod) => prod?._id === product?._id);
    ...
    <article className="flex flex-col gap-6 my-10">
      ...
      <article className="mt-10">
        ...
        <div className="flex flex-col gap-8">
          {products?.map((product) => (
            <ComparePrices key={product?._id} product={product} />
          ))}
        </div>
      </article>
    </article>
