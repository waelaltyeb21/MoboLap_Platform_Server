...
const CompoProduct = ({
  ...
  const HandleSelected = (val) => {
    console.log("Val: ", val);
    HandleSelectedProduct(id, val);
  };
  return (
    <div className="lg:col-span-4">
      <div className="relative w-full lg:h-32 flex justify-center items-center border-[1.5px] border-dashed hover:border-slate-600 rounded-lg p-4">
        {product ? (
          <h1>{product?.name}</h1>
        ) : (
          <Button
        ...
        {product && (
          <div className="absolute -top-3 -left-3">
            <X
              ...
              onClick={() => HandleRemoveProduct(id, product)}
            />
          </div>
        )}
      </div>
