...
const CartList = ({ products }) => {
  ...
  useEffect(() => {
    if (cartProducts?.length && products?.length) {
      console.log("Here");
      const AllProducts = cartProducts.map((cartItem) => {
        console.log("Cart Item: ", cartItem);
        const fullProduct = products.find((p) => p._id === cartItem._id);
      ...
      if (AllProducts) {
        CountTotal(AllProducts);
        setCartProductsList(AllProducts);
      }
    }
  ...
  const HandleRemove = (id) => {
    const UpdatedProducts = CartProductsList?.filter(
      (prod) => prod?._id !== id
    );

    // Update Current State
    setCartProductsList(UpdatedProducts);
    // Remove From Context State And Storage
    RemoveProductFromCart(id);
  };
  if (!CartProductsList) return <Loading />;
  return (
                ...
                <TableBody>
                  {CartProductsList?.map((product) => (
                    <TableRow key={product?._id} className="*:border-s *:p-4">
                      <TableCell>
                        {console.log(`${PRODUCT_IMAGE}/${product?.images[0]}`)}
                      ...
                      <TableCell className="text-center">
                        <X onClick={() => HandleRemove(product?._id)} />
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
