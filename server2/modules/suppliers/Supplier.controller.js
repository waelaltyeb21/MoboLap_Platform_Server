...
const GetSuppliers = async (req, res) => {
  try {
    ...
    console.log("Query: ", { city, category, status, sort });

    const filters = {};
    if (city && city != "الكل") filters["address.city"] = city;
...
const AddSupplier = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: "New Supplier Added Successfully" });
  } catch (err) {
    return res.status(500).json({ message: "Internal Server Error" });
  }
};
