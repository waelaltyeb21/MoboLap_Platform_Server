...
const CartList = () => {
  ...
  const RemoveProductFromCart = (id) => {
    const product = CartProducts?.find((prod) => prod?.id === id); // The Target Product
    const NewProducts = CartProducts?.filter((product) => product?.id !== id); // List Of Product Without The Removed One

    // Actionss
    toast.success("تم حذف المنتج من سلة الطلبات"); // Notify The User
